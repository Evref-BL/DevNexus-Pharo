...
function restorePluginMcpRoutingMetadata(
  projectRootPath: string,
  config: NexusProjectConfig,
): NexusProjectConfig {
  ...
  for (const plugin of config.plugins) {
    ...
    for (const capability of plugin.capabilities) {
      ...
      restoreOptionalString(capability, rawCapability, "transport", mcpTransportModes);
      ...
      restoreOptionalString(capability, rawCapability, "exposure", mcpExposureModes);
    }
  }
...
function persistPluginMcpRoutingMetadata(
  configPath: string,
  config: NexusProjectConfig,
): void {
  if (!config.plugins) {
  ...
  for (const plugin of config.plugins) {
    ...
    for (const capability of plugin.capabilities) {
      ...
      persistOptionalString(rawCapability, source, "transport", mcpTransportModes);
      ...
      persistOptionalString(rawCapability, source, "exposure", mcpExposureModes);
    }
  }
